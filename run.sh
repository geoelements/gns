...
python -m gns.train --device='cpu' --data_path=${DATA_PATH} --model_path=${MODEL_PATH} --ntraining_steps=10 --mode='train'

# Evaluate on validation split.
python -m gns.train --device='cpu' --data_path=${DATA_PATH} --model_path=${MODEL_PATH} --mode="eval" --eval_split="valid"

# Generate test rollouts.
ROLLOUT_PATH="${TMP_DIR}/rollouts/${DATASET_NAME}"
mkdir -p ${ROLLOUT_PATH}
python -m gns.train --device='cpu' --data_path=${DATA_PATH} --model_path=${MODEL_PATH} --output_path=${ROLLOUT_PATH} --mode='rollout'
