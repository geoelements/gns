...
class Processor(MessagePassing):
    """The Processor: $\mathcal{G} \rightarrow \mathcal{G}$ computes interactions among nodes
    via $M$ steps of learned message-passing, to generate a sequence of updated latent
    graphs, $G = (G_1 , ..., G_M ), where $G^{m+1| = GN^{m+1} (G^m )$.
    It returns the final graph, $G^M = PROCESSOR(G^0)$. Message-passing allows information to
    propagate and constraints to be respected: the number of message-passing steps required will
    likely scale with the complexity of the interactions."""
    ...
    def __init__(
        ...
        """Processor derived from torch_geometric MessagePassing class. The processor uses a stack
        of $M$ $GNs$ (where $M$ is a hyperparameter) with identical structure, MLPs as internal edge
        and node update functions, and either shared or unshared parameters. We use GNs without global features or
        global updates (i.e., an interaction network) , and with a residual connections between the
        input and output latent node and edge attributes.
    ...
    def forward(self,
                ...
                edge_index: torch.tensor,
                edge_features: torch.tensor):
        """The forward hook runs when the Processor class is instantiated. Run through GNN stacks

          Args:
            x: Particle state representation as a torch tensor with shape (nparticles, latent_dim)
            edge_index: A torch tensor list of source and target nodes with shape (2, nedges)
            edge_features: Edge features as a torch tensor with shape (nparticles, latent_dim)
